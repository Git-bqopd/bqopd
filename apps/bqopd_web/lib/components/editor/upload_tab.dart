...
import './modals/orphan_selector_modal.dart';
import './modals/confirm_modal.dart';

/// Decoupled Upload tab for adding new assets, selecting from the master library,
/// and compiling template pages directly into the active folio sequence.
class UploadTab extends StatefulComponent {
...
class _UploadTabState extends State<UploadTab> {
  ...
  void _triggerUpload() {
    triggerFilePicker('folio-uploader-native-picker', (base64, fileName, objectUrl) async {
      setState(() {
  ...
  Future<void> _addSelectedOrphans(List<Map<String, dynamic>> selected) async {
    ...
    try {
      ...
      print("Error adding selected orphans: $e");
      component.bloc.add(LoadFanzineRequested(component.fanzine.id));
    }
  }
...
""";
    ...
    repo.insertPublisherPage(component.fanzine.id, afterPageNum, initialText, component.pages).then((_) {
      ...
      print("Error creating text page: $e");
      component.bloc.add(LoadFanzineRequested(component.fanzine.id));
    });
  }
  ...
  void _triggerNewCalendarPage() {
    ...
    fsSetDoc('fanzines/${component.fanzine.id}/pages/$pageId', jsonEncode({
      ...
      print("Error creating calendar page: $e");
      component.bloc.add(LoadFanzineRequested(component.fanzine.id));
    });
  }
...
typedef EditorUploadTab = UploadTab;