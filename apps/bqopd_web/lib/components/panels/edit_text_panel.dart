...
/// Helper to calculate Levenshtein distance for AI baseline training data.
...
/// Dedicated inline drawer (bonusRow) editor for page text and wikilinks.
/// Compact styling optimized for accordion expansion within the reading feed.
class EditTextRowPanel extends StatefulComponent {
  final String imageId;
  final String? fanzineId;

  const EditTextRowPanel({
  ...
  @override
  State<EditTextRowPanel> createState() => _EditTextRowPanelState();
}

class _EditTextRowPanelState extends State<EditTextRowPanel> {
  String _textValue = '';
  ...
  @override
  void didUpdateComponent(EditTextRowPanel oldComponent) {
  ...
  Future<void> _loadText() async {
    ...
    try {
      ...
      if (doc['exists'] && mounted) {
        ...
        setState(() {
          _textValue = data['text_linked'] ?? data['text_corrected'] ?? data['text_raw'] ?? data['text'] ?? '';
      ...
      print('[EDIT TEXT ROW PANEL ERROR] Loading failed: $e');
      setState(() {
        _textValue = '';
        _loading = false;
      });
  ...
  Future<void> _saveText() async {
    ...
    try {
      ...
      if (_aiBaselineText.isNotEmpty) {
        final int score = calculateEditDistance(_aiBaselineText, _textValue);
      ...
      print('[EDIT TEXT ROW PANEL ERROR] Save failed: $e');
      if (mounted) {
        setState(() {
          _isSaving = false;
...
/// Dedicated desktop 3rd column (bonusColumn) editor for page text and wikilinks.
/// Rendered inside MultiPageColumnLayout card containers with expanded helper instructions.
class EditTextColumnPanel extends StatefulComponent {
  final String imageId;
  final String? fanzineId;

  const EditTextColumnPanel({
    required this.imageId,
    this.fanzineId,
    super.key,
  });
  ...
  @override
  State<EditTextColumnPanel> createState() => _EditTextColumnPanelState();
}

class _EditTextColumnPanelState extends State<EditTextColumnPanel> {
  String _textValue = '';
  String _aiBaselineText = '';
  bool _loading = true;
  bool _isSaving = false;
  ...
  @override
  void didUpdateComponent(EditTextColumnPanel oldComponent) {
    super.didUpdateComponent(oldComponent);
    if (oldComponent.imageId != component.imageId && kIsWeb) {
      _loadText();
    }
  ...
  Future<void> _loadText() async {
    ...
    try {
      ...
      print('[EDIT TEXT COLUMN PANEL ERROR] Loading failed: $e');
      setState(() {
        _textValue = '';
        _loading = false;
      });
  ...
  Future<void> _saveText() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _isSaving = false;
          _statusMessage = 'Saved page text and wikilinks!';
          _isError = false;
        });
        _resetStatusTimer();
      }
    } catch (e) {
      print('[EDIT TEXT COLUMN PANEL ERROR] Save failed: $e');
      if (mounted) {
        setState(() {
          _isSaving = false;
          _statusMessage = 'Failed to save: ${e.toString()}';
    ...
    if (_loading) {
      return div(
        [
          div([], classes: 'skeleton-line shimmer-bg', attributes: const {'style': 'height: 12px; border-radius: 4px; width: 100%;'}),
          div([], classes: 'skeleton-line medium shimmer-bg', attributes: const {'style': 'height: 12px; border-radius: 4px; width: 85%;'}),
          div([], classes: 'skeleton-line shimmer-bg', attributes: const {'style': 'height: 12px; border-radius: 4px; width: 70%;'}),
        ],
        classes: 'flex-col gap-2 py-4',
        attributes: const {'style': 'display: flex; flex-direction: column; gap: 8px; width: 100%;'},
      );
    ...
    return div(
      [
        div(
            ...
            attributes: const {
              'style': 'display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 8px; width: 100%;'
            }
        ),
        div(
          [
...
typedef EditTextPanel = EditTextRowPanel;