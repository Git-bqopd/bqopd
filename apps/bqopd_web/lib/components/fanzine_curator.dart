...
class _FanzineCuratorState extends State<FanzineCurator> {
  ...
  Component build(BuildContext context) {
    ...
    if (state is FanzineEditorLoading || state is FanzineEditorInitial) {
      return div(
        [
          p([Component.text("Synchronizing curator workspace...")])
        ],
    ...
    if (state is FanzineEditorFailure) {
      return div(
        [
          h3([Component.text("Curator Interface Failure")], attributes: const {'style': 'color: #ff5252; margin: 0 0 8px 0;'}),
...
class _EditorOcrEntitiesTabState extends State<EditorOcrEntitiesTab> {
      ...
      [
        ...
        div(
          ...
          [
            button(
              [
                span([Component.text('auto_fix_high')], classes: 'material-symbols-outlined', attributes: const {'style': 'font-size: 16px; margin-right: 6px;'}),
                Component.text("Step 2: AI Clean")
              ],
              classes: 'profile-btn',
            ...
            button(
              [
                span([Component.text('link')], classes: 'material-symbols-outlined', attributes: const {'style': 'font-size: 16px; margin-right: 6px;'}),
                Component.text("Step 3: Generate Links")
              ],
              classes: 'profile-btn',
        ...
        h3([Component.text("Detected Entities Archive")], classes: 'text-xs font-bold text-gray uppercase tracking-wider mb-1'),
        if (draftEntities.isEmpty)
          p([Component.text("No entities registered in this issue's index pipeline yet.")], classes: 'text-xs text-gray italic text-center py-4')
        else
          div(
  ...
  Component _buildCounterSquare(String label, int val, String color) {
    return div(
      ...
      [
        span([Component.text(_calculating ? "..." : "$val")], attributes: {
          'style': 'font-size: 24px; font-weight: 900; color: $color; line-height: 1;'
        }),
        span([Component.text(label.toLowerCase())], attributes: const {
...
class _OcrWorkspaceEntityRowState extends State<OcrWorkspaceEntityRow> {
  ...
  Component build(BuildContext context) {
    if (_loading) {
      return div(
          [span([Component.text("syncing... @${normalizeHandle(component.name)}")], attributes: const {'style': 'font-style: italic; color: #999; font-size: 12px;'})],
    ...
    return div(
      ...
      [
        div(
          ...
          [
            span([Component.text(component.name)], attributes: const {'style': 'font-weight: bold; font-size: 13px; color: black;'}),
            if (_isAlias && _redirectHandle != null)
              span([Component.text("aliases to @$_redirectHandle")], attributes: const {'style': 'font-size: 10px; color: #2563eb; font-weight: bold;'})
          ],
        ),
        div(
          [
            if (_exists)
              a(
                  [
                    span([Component.text('account_circle')], classes: 'material-symbols-outlined', attributes: const {'style': 'font-size: 14px; margin-right: 4px; vertical-align: middle;'}),
              ...
              div(
                ...
                [
                  ...
                  button(
                      [span([Component.text('done')], classes: 'material-symbols-outlined', attributes: const {'style': 'font-size: 13px;'})],
                      attributes: const {'style': 'border: none; background: transparent; color: #16a34a; cursor: pointer;'},
                      events: {'click': (e) => _submitAliasRedirect()}
                  ),
                  button(
                      [span([Component.text('close')], classes: 'material-symbols-outlined', attributes: const {'style': 'font-size: 13px;'})],
