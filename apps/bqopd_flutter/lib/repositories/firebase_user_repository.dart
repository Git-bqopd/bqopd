...
class FirebaseUserRepository implements IUserRepository {
  ...
  Stream<UserProfile?> watchUser(String uid) {
    return _db.collection('profiles').doc(uid).snapshots().map(
          (doc) => doc.exists
  ...
  Stream<UserAccount?> watchUserAccount(String uid) {
    return _db.collection('Users').doc(uid).snapshots().map(
          (doc) => doc.exists
          ? UserAccount.fromMap(doc.id, doc.data() as Map<String, dynamic>)
          : null,
    );
  }
  ...
  Stream<List<Map<String, dynamic>>> watchUserMentions(String uid) {
    ...
    _db.collection('profiles').doc(uid).get().then((profileSnap) {
      ...
      final subscription =
      _db.collection('fanzines').snapshots().listen((snapshot) {
        ...
        for (var doc in snapshot.docs) {
          ...
          if (!isMentioned) {
            ...
            final lowerEntities = draftEntities
                .map((e) => e.toString().toLowerCase().trim())
                .toList();
            if (displayName.isNotEmpty &&
                lowerEntities.contains(displayName.toLowerCase())) {
              isMentioned = true;
            }
            if (!isMentioned &&
                username.isNotEmpty &&
                lowerEntities.contains(username.toLowerCase())) {
              isMentioned = true;
            }
          }
        ...
        filtered.sort((a, b) {
          final aT =
              a['publishedAt'] ?? a['creationDate'] ?? a['createdAt'] ?? 0;
          final bT =
              b['publishedAt'] ?? b['creationDate'] ?? b['createdAt'] ?? 0;
