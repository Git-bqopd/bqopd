...
class BasicTextTemplate extends StatelessWidget {
  ...
  static const double textContentHeight =
      targetHeight - (outerBorderWidth * 2) - (innerPadding * 2);
  ...
  static List<List<List<PageBlock>>> paginateContent(String fullText) {
    ...
    for (int i = 0; i < rawTokens.length; i++) {
      if (rawTokens[i].isNotEmpty) {
        layoutTokens.add(rawTokens[i]);
      }
      if (i < matches.length) {
          ...
          if (currentColumnHeight + textPainter.height <= textContentHeight + 0.5) {
            ...
            if (pendingText.isNotEmpty) {
              currentColumnBlocks.add(
                PageBlock(type: BlockType.text, content: pendingText),
              );
            }
            currentColumns.add(currentColumnBlocks);
            currentColumnBlocks = [];
