...
class _FanzineMakerWidgetState extends State<FanzineMakerWidget> {
  ...
  Widget _buildMakerOrderTab(BuildContext context, Fanzine fanzine, List<FanzinePage> pages) {
                ...
                ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    Widget layoutButtonsWidget = isCompact
                        ? (showLayoutButtons
                        ? Align(alignment: Alignment.centerLeft, child: FittedBox(fit: BoxFit.scaleDown, child: layoutRow))
                        : const SizedBox.shrink())
                        : SizedBox(
                      width: 280,
                      child: showLayoutButtons
                          ? FittedBox(fit: BoxFit.scaleDown, alignment: Alignment.centerRight, child: layoutRow)
                          : null,
                    );
                        ...
                        children: [
                          Row(
                            children: [
                              SizedBox(
                                width: 24,
                                child: Text('$num.', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 11)),
                              ),
                              Expanded(
                                child: Text(
                                  page.templateId != null ? "template page" : "image page",
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: page.imageUrl == null
                            ? const Center(child: Icon(Icons.auto_awesome_motion, size: 16, color: Colors.grey))
                            : null,
                      ),
                    );
                  },
...
class _MakerUploadTabState extends State<_MakerUploadTab> {
        ...
        if (w == null || h == null) {
          try {
            ...
            FirebaseFirestore.instance
                .collection('images')
                .doc(img['id'])
                .update({'width': w, 'height': h, 'aspectRatio': w / h});
          } catch (_) {}
        }
  ...
  String _getBadgeLabel(Map<String, dynamic> data) {
    ...
    if (context != null && context != widget.fanzineId) {
      return _folioNames[context] ?? "other folio";
    }
    if (context == null && usedIn.contains(widget.fanzineId)) {
      return "orphan";
    }
    if (context == widget.fanzineId) {
      return widget.folioTitle;
    }
    return "orphan";
  }
...
class _AssetEditModalState extends State<_AssetEditModal> {
  ...
  Future<void> _saveTitle() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('title saved!')));
      }
    } finally {
      if (mounted) {
      ...
      child: ConstrainedBox(
        ...
        child: LayoutBuilder(
          builder: (context, constraints) {
            ...
            return isMobile
                ? Column(children: [imgWidget, Expanded(child: editWidget)])
                : Row(children: [Expanded(child: imgWidget), Expanded(child: editWidget)]);
          },
        ),
      ),
    );
