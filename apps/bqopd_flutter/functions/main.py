...
# --------------------------------------------------------------------------------
# CONSOLIDATED WORKER 2: AI FORMATTING, CORRECTION & ENTITY WIKILINKING
# --------------------------------------------------------------------------------
...
def ai_cleaning_worker(event: firestore_fn.Event[firestore_fn.Change[firestore_fn.DocumentSnapshot]]) -> None:
    ...
    try:
        ...
        event.data.after.reference.update({
            ...
            'text_corrected_ai': text_linked, # Your unified initial snapshot field!
            'text_linked': text_linked,
            'detected_entities': clean_ents,
            'needs_ai_cleaning': False,
            'needs_linking': False # Single pass completes the entire routine
        })
...
# --------------------------------------------------------------------------------
# LEGACY WORKER 3: LINKING WORKER (DISABLED AND MOVED TO COMBINED TRANSACTIONS)
# --------------------------------------------------------------------------------
@firestore_fn.on_document_written(document="images/{imageId}", secrets=[GEMINI_API_KEY], memory=1024, timeout_sec=120)
def linking_worker(event: firestore_fn.Event[firestore_fn.Change[firestore_fn.DocumentSnapshot]]) -> None:
    return # Legacy stub - combined cleanly into the single-pass ai_cleaning_worker above!

# --------------------------------------------------------------------------------
# WORKER 4: THUMBNAILS, SIZES, AND FILE EXPORTS
# --------------------------------------------------------------------------------
...
def trigger_generate_links(req: https_fn.CallableRequest):
    # Simply forward to triggers to run the new unified cleanup worker
    return trigger_ai_clean(req)
